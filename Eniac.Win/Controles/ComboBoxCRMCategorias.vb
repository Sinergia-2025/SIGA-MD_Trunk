Public Class ComboBoxCRMCategorias
   Inherits ComboBoxMultipleSeleccion2

   Private _listaCategorias As List(Of Entidades.CRMCategoriaNovedad)
   ...
   Public Sub New()
      MyBase.New()
      _listaCategorias = New List(Of Entidades.CRMCategoriaNovedad)()
   End Sub
   ...
   End Function
   Public Function GetCategorias(idTipoNovedad As String, Optional ordenarPosicion As Boolean = False) As Entidades.CRMCategoriaNovedad()
      Return GetCategorias(False, idTipoNovedad)
   End Function
   Public Function GetCategorias(todosVacio As Boolean, idTipoNovedad As String) As Entidades.CRMCategoriaNovedad()
      If Convert.ToInt32(SelectedValue) = Convert.ToInt32(Entidades.Sucursal.ValoresFijosIdSucursal.Todos) Then
         ...
         Return MFCRMCategorias.GetTodas(idTipoNovedad:=idTipoNovedad, ordenarPosicion:=False).ToArray()
      End If
   ...
   Public Sub Inicializar(permiteSinSeleccion As Boolean, seleccionMultiple As Boolean, seleccionTodos As Boolean, Optional idTipoNovedad As String = "")
      ...
      _publicos.CargaComboCRMCategoriasNovedades(Me, seleccionMultiple, seleccionTodos, idTipoNovedad)
      Refrescar()
      _initialized = True
      _permiteSinSeleccion = permiteSinSeleccion
      _listaCategorias.Clear()
      _listaCategorias.Add(DirectCast(SelectedItem, Entidades.CRMCategoriaNovedad))
   ...
   Protected Overrides Sub OnSelectedIndexChanged(e As EventArgs)
      ...
      If _initialized And Enabled Then
         If SelectedIndex > -1 Then
            Using filtromultiplecomprobantes As New MFCRMCategorias(_permiteSinSeleccion, _idTipoNovedad)
               If Convert.ToInt32(SelectedValue) = Convert.ToInt32(Entidades.Sucursal.ValoresFijosIdSucursal.SeleccionMultiple) Then
                  If _listaCategorias.Any() AndAlso _listaCategorias(0).IdCategoriaNovedad = Convert.ToInt32(Entidades.Sucursal.ValoresFijosIdSucursal.Todos) Then
                     filtromultiplecomprobantes.Filtros.AddRange(MFCRMCategorias.GetTodas(idTipoNovedad:=_idTipoNovedad, ordenarPosicion:=False))
                  Else
                  ...
                  If _listaCategorias.Count = 1 Then
                     SelectedValue = _listaCategorias(0).IdCategoriaNovedad
                  End If
   ...
   Private Overloads Sub CargarOpcionesContextuales()
      CargarOpcionesContextuales(MFCRMCategorias.TipoFiltroKey)
   End Sub

   Protected Overrides Sub CargaFiltrosGuardados(nombreFiltro As String)
      Try
         Using frm As MFCRMCategorias = New MFCRMCategorias(_permiteSinSeleccion, _idTipoNovedad)
