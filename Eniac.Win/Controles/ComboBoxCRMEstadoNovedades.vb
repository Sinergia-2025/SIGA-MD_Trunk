Public Class ComboBoxCRMEstadoNovedades
   Inherits ComboBoxMultipleSeleccion2

   Private _listaEstados As List(Of Entidades.CRMEstadoNovedad)
   ...
   Public Sub New()
      MyBase.New()
      _listaEstados = New List(Of Entidades.CRMEstadoNovedad)()
   End Sub
   ...
   End Function
   Public Function GetEstado(idEstadoNovedad As String, Optional ordenarPosicion As Boolean = False) As Entidades.CRMEstadoNovedad()
      Return GetEstado(False, idEstadoNovedad)
   End Function
   Public Function GetEstado(todosVacio As Boolean, idTipoNovedad As String) As Entidades.CRMEstadoNovedad()
      If Convert.ToInt32(SelectedValue) = Convert.ToInt32(Entidades.Sucursal.ValoresFijosIdSucursal.Todos) Then
         ...
         Return MFCRMEstados.GetTodas(idTipoNovedad:=idTipoNovedad, ordenarPosicion:=False).ToArray()
      End If
      Return _listaEstados.ToArray()
   End Function
   ...
   Public Sub Inicializar(permiteSinSeleccion As Boolean, seleccionMultiple As Boolean, seleccionTodos As Boolean, Optional idTipoNovedad As String = "")
      ...
      _publicos.CargaComboCRMEstadosNovedades(Me, seleccionMultiple, seleccionTodos, idTipoNovedad)
      Refrescar()
      _initialized = True
      _permiteSinSeleccion = permiteSinSeleccion
      _listaEstados.Clear()
      _listaEstados.Add(DirectCast(SelectedItem, Entidades.CRMEstadoNovedad))
   ...
   Protected Overrides Sub OnSelectedIndexChanged(e As EventArgs)
      ...
      If _initialized And Enabled Then
         If SelectedIndex > -1 Then
            Using filtromultiplecomprobantes As New MFCRMEstados(_permiteSinSeleccion, _idTipoNovedad)
               If Convert.ToInt32(SelectedValue) = Convert.ToInt32(Entidades.Sucursal.ValoresFijosIdSucursal.SeleccionMultiple) Then
                  If _listaEstados.Any() AndAlso _listaEstados(0).IdEstadoNovedad = Convert.ToInt32(Entidades.Sucursal.ValoresFijosIdSucursal.Todos) Then
                     filtromultiplecomprobantes.Filtros.AddRange(MFCRMEstados.GetTodas(idTipoNovedad:=_idTipoNovedad, ordenarPosicion:=False))
                  Else
                     filtromultiplecomprobantes.Filtros.AddRange(_listaEstados)
                  End If
                  ...
                  If filtromultiplecomprobantes.Filtros.Count = 0 Then
                     SelectedIndex = -1
                  Else
                     _listaEstados.AddRange(filtromultiplecomprobantes.Filtros)
                  End If
                  If _listaEstados.Count = 1 Then
                     SelectedValue = _listaEstados(0).IdEstadoNovedad
                  End If
   ...
   Public Overrides Function GetDefaultParametrosImpresion() As CargaFiltroImpresionParams
      Return New CargaFiltroImpresionParams("Estado", "Estados")
   End Function

   Private Overloads Sub CargarOpcionesContextuales()
      CargarOpcionesContextuales(MFCRMEstados.TipoFiltroKey)
   End Sub

   Protected Overrides Sub CargaFiltrosGuardados(nombreFiltro As String)
      Try
         Using frm As MFCRMEstados = New MFCRMEstados(_permiteSinSeleccion, _idTipoNovedad)
