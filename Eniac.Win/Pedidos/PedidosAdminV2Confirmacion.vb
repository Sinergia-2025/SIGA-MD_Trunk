Public Class PedidosAdminV2Confirmacion
...
#Region "Overrides"
   ...
   Public Overloads Function ShowDialog(owner As IWin32Window, dt As DataTable) As DialogResult
      dt.Columns.Add("Deposito", GetType(String))
      dt.Columns.Add("Ubicacion", GetType(String))
      dt.Columns.Add("Lote", GetType(String))
      dt.Columns.Add("Nro Serie", GetType(String))
      _dtDetalle = dt
   ...
   Protected Overrides Sub OnLoad(e As EventArgs)
      ...
      Try
         ...
         ''Me._publicos.CargaComboEstadosPedidos(cmbEstados, True, True, True, False, False, _tipoTipoComprobante)
         ...
         Me._publicos.CargaComboEstadosPedidos(cmbEstadoCambiar, False, False, False, False, False, _tipoTipoComprobante)

         'Me.cmbEstadoCambiar.Items.Insert(0, "EN PROCESO")
         'Me.cmbEstadoCambiar.Items.Insert(1, "ANULADO")
         'Me.cmbEstadoCambiar.Items.Insert(2, "FINALIZADO")
         ...
         Me.ugDetalle.DisplayLayout.Override.RowSelectorHeaderStyle = RowSelectorHeaderStyle.ColumnChooserButton

         Me.ugDetalle.DisplayLayout.Override.RowSelectors = DefaultableBoolean.True
         Me.cmbEstadoCambiar.SelectedIndex = 0

         Me.ugDetalle.DisplayLayout.Bands(0).Columns("ClipArchivoAdjunto").Hidden = Not Publicos.DetallePedido.PedidosMostrarUrlPlanoDetalle
         ...
         ugDetalle.DisplayLayout.Bands(0).Columns("Nota").Hidden = Not Publicos.DetallePedido.PedidosMostrarNota

         ugDetalle.DisplayLayout.Bands(0).Columns("CodigoSAE").Hidden = Not Publicos.DetallePedido.PedidosMostrarProductoSAE
         ugDetalle.DisplayLayout.Bands(0).Columns("LargoExtAlto").Hidden = Not Publicos.DetallePedido.PedidosMostrarProductoLargoExtAlto
         ugDetalle.DisplayLayout.Bands(0).Columns("AnchoIntBase").Hidden = Not Publicos.DetallePedido.PedidosMostrarProductoAnchoIntBase
         ugDetalle.DisplayLayout.Bands(0).Columns("UrlPlano").Hidden = Not Publicos.DetallePedido.PedidosMostrarUrlPlanoDetalle

         ''Me._publicos.CargaComboZonasGeograficas(Me.cmbZonaGeografica)
         ''Me.cmbZonaGeografica.SelectedIndex = -1


         Ayudante.Grilla.AgregarFiltroEnLinea(ugDetalle, {"NombreCliente", "NombreProducto", "observacion", "Direccion", "Telefono"}, {"ClipArchivoAdjunto"})

         Dim oUsuario As Eniac.Reglas.Usuarios = New Eniac.Reglas.Usuarios()
         ''tsbModificarPedido.Visible = oUsuario.TienePermisos(Eniac.Ayudantes.Common.usuario,
         ''                                                    Eniac.Entidades.Usuario.Actual.Sucursal.Id,
         ''                                                    "PedidosAdmin-Modif")
...
#Region "Eventos"
   ...
   ''Private Sub chbVendedor_CheckedChanged(sender As System.Object, e As EventArgs)
   ...
   ''Private Sub chbMarca_CheckedChanged(sender As System.Object, e As EventArgs)
   ''   Try
   ...
   ''Private Sub chbRubro_CheckedChanged(sender As System.Object, e As EventArgs)
   ''   Try
   ...
   ''Private Sub chbSubRubro_CheckedChanged(sender As System.Object, e As EventArgs)
   ''   Try
   ...
   Private Sub PedidosAdmin_KeyUp(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyUp
   ...
   Private Sub tsbImprimirPredisenado_Click(sender As System.Object, e As EventArgs) Handles tsbImprimirPredisenado.Click
      Try
         Me.Cursor = Cursors.WaitCursor
   ...
   ''Private Sub chbFecha_CheckedChanged(sender As System.Object, e As EventArgs)
   ...
   ''Private Sub chkMesCompleto_CheckedChanged(sender As System.Object, e As EventArgs)
   ...
   ''Private Sub chbProducto_CheckedChanged(sender As System.Object, e As EventArgs)
   ...
   ''Private Sub bscCodigoProducto2_BuscadorClick()
   ''   Try
   ...
   ''Private Sub bscCodigoProducto2_BuscadorSeleccion(sender As Object, e As Controles.BuscadorEventArgs)
   ''   Try
   ...
   ''Private Sub bscProducto2_BuscadorClick()
   ''   Try
   ''      Dim oProductos As Reglas.Productos = New Reglas.Productos
   ...
   ''Private Sub bscProducto2_BuscadorSeleccion(sender As Object, e As Controles.BuscadorEventArgs)
   ''   Try
   ''      If Not e.FilaDevuelta Is Nothing Then
   ''         Me.CargarProducto(e.FilaDevuelta)
   ''      End If
   ...
   ''Private Sub bscCliente_BuscadorClick()
   ''   Try
   ...
   ''Private Sub bscCliente_BuscadorSeleccion(sender As Object, e As Controles.BuscadorEventArgs)
   ''   Try
   ''      If Not e.FilaDevuelta Is Nothing Then
   ''         Me.CargarDatosCliente(e.FilaDevuelta)
   ''      End If
   ...
   ''Private Sub chbCliente_CheckedChanged(sender As System.Object, e As EventArgs)
   ...
   ''Private Sub chbIdPedido_CheckedChanged(sender As System.Object, e As EventArgs)
   ...
   ''Private Sub chbTamanio_CheckedChanged(sender As System.Object, e As EventArgs)
   ...
   ''Private Sub btnConsultar_Click(sender As System.Object, e As EventArgs)
   ...
   ''Private Sub chkExpandAll_CheckedChanged(sender As System.Object, e As EventArgs)
   ...
   Private Sub cmdMasivo_Click(sender As System.Object, e As EventArgs) Handles cmdMasivo.Click
      Try
         ...
         If cmbResponsable.Visible AndAlso cmbResponsable.SelectedIndex = -1 Then
            ShowMessage(String.Format("El estado {0} genera una Orden de Producción. Debe seleccionar un Responsable.", cmbEstadoCambiar.SelectedText))
            Exit Sub
         End If
         ...
         If respuesta = Windows.Forms.DialogResult.Yes Then
               ...
               If Boolean.Parse(fila("masivo").ToString) Then
                  ...
                  If tipoComprobanteEstadoCambia IsNot Nothing AndAlso tipoComprobanteEstadoCambia.Tipo = Entidades.TipoComprobante.Tipos.PRODUCCION.ToString() Then
                     If Not IsNumeric(fila("IdFormula")) Then
                        mensage += String.Format(Traducciones.TraducirTexto("Pedido: {0} --> El Producto {1} - '{2}' no tiene definida una Formula de Producción. No es posible pasar al módulo de Producción.", Me, "__ErrorCambioMasivoNoPermitido"), fila("numeropedido"), fila("IdProducto"), fila("NombreProducto")) & vbCrLf
                        graba = False
                     End If
                  End If
            ...
            'For Each row As DataRow In tablaGrabar.Rows
            '   row.Item("IdCriticidad") = Me.cmbCriticidad.Text
            '   row.Item("FechaEntrega") = dtpFechaEntrega.Value
            'Next

            'Dim IdCriticidad As String = cmbCriticidad.Text
            'Dim FechaEntrega As DateTime = dtpFechaEntrega.Value
   ...
   Private Sub cmbEstadoCambiar_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles cmbEstadoCambiar.KeyDown
      If e.KeyCode = Keys.Enter Then
         Me.ProcessTabKey(True)
      End If
   End Sub

   Private Sub cmbResponsable_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles cmbResponsable.KeyDown
      If e.KeyCode = Keys.Enter Then
         Me.ProcessTabKey(True)
      End If
   End Sub
   ...
   Private Sub ugDetalle_InitializeLayout(sender As System.Object, e As Infragistics.Win.UltraWinGrid.InitializeLayoutEventArgs) Handles ugDetalle.InitializeLayout
...
#End Region
   ...
   Private Sub cmbCriticidad_KeyDown(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles cmbCriticidad.KeyDown
      If e.KeyCode = Keys.Enter Then
         Me.ProcessTabKey(True)
      End If
   End Sub
   ...
   Private Sub dtpFechaEntrega_KeyDown(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles dtpFechaEntrega.KeyDown
      If e.KeyCode = Keys.Enter Then
         Me.ProcessTabKey(True)
      End If
   End Sub

   ''Private Sub tsbModificarPedido_Click(sender As Object, e As EventArgs)
   ''   Try
   ...
   ''Private Sub tsbCopiarPedido_Click(sender As Object, e As EventArgs)
   ''   Try
   ''      Me.Cursor = Cursors.WaitCursor
   ...
   ''Private Sub tsbConvertirEnFactura_Click(sender As Object, e As EventArgs)
   ''   Try
   ''      If ugDetalle.ActiveRow IsNot Nothing AndAlso
   ''         ugDetalle.ActiveRow.ListObject IsNot Nothing AndAlso
   ''         TypeOf (ugDetalle.ActiveRow.ListObject) Is DataRowView AndAlso
   ...
   ''Private Sub chbOrdenCompra_CheckedChanged(sender As Object, e As EventArgs)
   ''   Try
   ...
   ''Private Sub ugdDetalle_ClickCellButton(sender As System.Object, e As Infragistics.Win.UltraWinGrid.CellEventArgs) Handles ugDetalle.ClickCellButton
   ''   Try
