Public Class CmdSubidaRecibosPDF
   Implements Eniac.Win.IComandoMenu

   Private Const Tag As String = "CmdSubidaRecibosPDF.Ejecutar"
   ...
   Public Sub Ejecutar(owner As IWin32Window, idFuncion As String) Implements IComandoMenu.Ejecutar
      ...
      My.Application.Log.WriteEntry(String.Format("{0} - Inicio subida de Comprobantes de Recibos.", Tag), TraceEventType.Verbose)
      Dim ftp As Reglas.IFtp = New Reglas.SimpleFtp(servidorFTP, usuarioFTP, claveFTP)

      '# Buscando la función de exportación de Recibos
      My.Application.Log.WriteEntry(String.Format("{0} - Buscando función de Exportación de Recibos", Tag), TraceEventType.Verbose)

      Dim funcion As Entidades.Funcion = New Reglas.Funciones().GetUna("CmdExportacionRecibos", False, cargaVinculadas:=False)
      If funcion Is Nothing Then
         My.Application.Log.WriteEntry(String.Format("{0} - No existe la función de exportación de Recibos", Tag), TraceEventType.Verbose)
         My.Application.Log.WriteEntry(String.Format("{0} - Finaliza", Tag), TraceEventType.Information)
         Exit Sub
      End If

      '# Una vez obtenida la función de Recibos, busco la ubicación de la exportación
      My.Application.Log.WriteEntry(String.Format("{0} - Buscando ubicación de las Exportaciones de Recibos", Tag), TraceEventType.Verbose)
      ...
      '# Valido que exista la ubicación de donde tomar los comprobantes.
      ...
      My.Application.Log.WriteEntry(String.Format("{0} - Fin subida de Comprobantes de Recibos.", Tag), TraceEventType.Verbose)
   End Sub
End Class
