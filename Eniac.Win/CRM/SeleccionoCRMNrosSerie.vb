...
Public Class SeleccionoCRMNrosSerie
   Private _titNumeroSerie As Dictionary(Of String, String)
   Private _cantidadDeProductos As Integer
   Private _producto As Entidades.CRMNovedadProducto
   Private _productosNrosSerie As List(Of Entidades.CRMNovedadProductoNrosSerie)
   Private _dtNroSerie As New DataTable
   Public _IdDeposito As Integer = 0
   Public _IdUbicacion As Integer = 0

   Public ReadOnly Property ProductosNrosSerie() As List(Of Entidades.CRMNovedadProductoNrosSerie)
      Get
         Return _productosNrosSerie
      End Get
   End Property

   Public Sub New(ByVal cantidadDeProductos As Integer, ByVal producto As Entidades.CRMNovedadProducto, ByVal dt As DataTable)
      MyBase.New()
      InitializeComponent()

      Me._productosNrosSerie = New List(Of Entidades.CRMNovedadProductoNrosSerie)()
      ...
      If producto.ProductosNovedadNrosSerie.Count > 0 Then
         For Each dr As DataRow In _dtNroSerie.Rows
            For Each ns As Entidades.CRMNovedadProductoNrosSerie In producto.ProductosNovedadNrosSerie
   ...
   Private Sub btnAceptar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAceptar.Click
      Try
         ...
         Dim pns As Entidades.CRMNovedadProductoNrosSerie
         ...
         For Each dr As DataRow In DirectCast(Me.ugNrosSerie.DataSource, DataTable).Select("Selec = True")
            If cant <= Math.Abs(_cantidadDeProductos) Then
               pns = New Entidades.CRMNovedadProductoNrosSerie()
               pns.IdProducto = Me._producto.IdProducto
               ' pns.Vendido = True
               pns.IdSucursal = New Reglas.Sucursales().GetUna(Entidades.Usuario.Actual.Sucursal.IdSucursal, False).IdSucursal
