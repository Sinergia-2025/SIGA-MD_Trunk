Public Class PagoMisCuentasProceso
   Implements IDebitosDirectosProceso

   Private _add As ArchivoFacturaPMC
   ...
   Public Sub CrearDD(dt As System.Data.DataTable,
      ...
      Try
         Me._add = New ArchivoFacturaPMC()

         Dim linea As ArchivoFacturaPMCDatos
         ...
         With Me._add
            ...
            For Each dr As DataRow In dt.Select("Selec = True")
               ...
               linea = New ArchivoFacturaPMCDatos()

               With linea

                  ' Identificación del cliente en la empresa. 
                  .NroReferencia = dr("ReferenciaPMC").ToString()

                  Dim fecha As Date
                  fecha = fechaVencimiento.AddDays(-30)

                  ' Identificación de la factura.       8 pos tipo comprobante, 1 pos letra, 3 pos emisor, 8 pos nro comprobante
                  .IdFactura = dr("IdTipoComprobante").ToString().PadRight(8) & dr("Letra").ToString() & Integer.Parse(dr("CentroEmisor").ToString()).ToString("000") & Long.Parse(dr("NumeroComprobante").ToString()).ToString("00000000")

                  ' Fecha del 1er vencimiento de la factura
                  .Fecha1erVencimiento = fechaVencimiento
                  ...
                  .Fecha2doVencimiento = fechaVencimiento

                  ' Importe de la factura para el 2do vencimiento.
                  .Importe2doVencimiento = Decimal.Parse(dr("Saldo").ToString())

                  ' Fecha del 3er vencimiento de la factura
                  .Fecha3erVencimiento = fechaVencimiento

                  ' Importe de la factura para el 3er vencimiento.
                  .Importe3erVencimiento = Decimal.Parse(dr("Saldo").ToString())
