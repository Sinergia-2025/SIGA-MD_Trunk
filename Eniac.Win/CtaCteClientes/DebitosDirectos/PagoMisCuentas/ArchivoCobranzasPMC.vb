...
#End Region
Public Class ArchivoCobranzasPMC
...
#Region "Propiedades"
   ...
   ''' <returns></returns>
   ''' <remarks></remarks>
   Public Property FecharArchivo() As DateTime
      Get
   ...
   Private _datos As List(Of ArchivoCobranzasPMCDatos)
   Public Property Datos() As List(Of ArchivoCobranzasPMCDatos)
      Get
         If Me._datos Is Nothing Then
            Me._datos = New List(Of ArchivoCobranzasPMCDatos)()
         End If
         Return _datos
      End Get
      Set(ByVal value As List(Of ArchivoCobranzasPMCDatos))
         _datos = value
      End Set
   End Property
   ...
   Public ReadOnly Property TotalImporte As Decimal
      Get
         ...
         For Each ar As ArchivoCobranzasPMCDatos In Datos
            tot += ar.Importe
         Next
         Return tot
      End Get
...
#End Region
   ...
   Public Function GetInfo(archivoFisico As StreamReader) As ArchivoCobranzasPMC

      Dim linea As String
      Dim arDa As ArchivoCobranzasPMC = New ArchivoCobranzasPMC()
      ...
      'TODO: DB
      ' ''If Me.CodigoEmpresa <> Publicos.NumeroEmpresaBanelco Then
      ' ''    Throw New Exception(String.Format("El archivo que está importando pertenece a otra empresa.{0}{0}Su codigo empresa: {1}{0}Empresa del archivo: {2}",
      ' ''                                      Environment.NewLine, Publicos.NumeroEmpresaBanelco, Me.CodigoEmpresa))
      ' ''End If

      'Fecha de generación del archivo. Debe coincidir con la fecha del nombre del archivo
      'Formato: AAAAMMDD
      Me.FecharArchivo = New DateTime(Int32.Parse(linea.Substring(8, 4)), Int32.Parse(linea.Substring(12, 2)), Int32.Parse(linea.Substring(14, 2)))

      linea = archivoFisico.ReadLine()
      Do While linea IsNot Nothing
         If linea.Substring(0, 1) = "5" Then 'es una detalle
            Me.Datos.Add(New ArchivoCobranzasPMCDatos().GetDato(linea))
         Else
            ' = 9 then Trailer
...
Public Class ArchivoCobranzasPMCDatos
...
#Region "Metodos Publicos"
   ...
   Public Function GetDato(linea As String) As ArchivoCobranzasPMCDatos
      Dim dato As ArchivoCobranzasPMCDatos = New ArchivoCobranzasPMCDatos()
      ...
      With dato
         ...
         oCliente = New Reglas.Clientes().GetUnoPorTipoYNroDocumento("DNI", Long.Parse(dato.NroReferencia.ToString()))
         If oCliente Is Nothing Then
            oCliente = New Reglas.Clientes().GetUnoPorCuit(dato.NroReferencia.ToString())
         End If
         If oCliente Is Nothing Then
            Throw New Exception("Una linea contiene el DNI " & dato.NroReferencia.ToString.Trim() & " que NO Existe en el sistema. NO podra continuar. Detalle: " & linea)
            Exit Function
         End If
