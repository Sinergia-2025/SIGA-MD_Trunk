...
Public Class SueldosModificarLiquidacion
...
#Region "Eventos"
   ...
   Private Sub tsbModificarLugaryFechaPago_Click(sender As Object, e As EventArgs) Handles tsbModificarLugaryFechaPago.Click
      Try
         If cmbNroLiquidacion.Items.Count <> 0 Then
            Dim IdLegajo As Integer = 0

            If Me.chbSocio.Checked Then
               If Me.bscNroLegajo.Selecciono Then
                  IdLegajo = Integer.Parse(Me.bscNroLegajo.Text.ToString())
               Else
                  MessageBox.Show("Debe seleccionar Legajo.", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Information)
                  Exit Sub
               End If
            End If

            If MessageBox.Show("Confirma modificar datos para la Liquidación: " & Me.cmbPeriodoLiquidacion.Text.ToString() & " - Tipo de Recibo: " & cmbTipoRecibo.Text & " - Nro Liquidación: " & cmbNroLiquidacion.Text + IIf(Me.chbSocio.Checked, " - Legajo: " & Me.bscNroLegajo.Text, "").ToString(), Me.Text, MessageBoxButtons.OKCancel,
                                       MessageBoxIcon.Warning) = Windows.Forms.DialogResult.OK Then



               Dim oLiquidacion As Reglas.SueldosLiquidacion = New Reglas.SueldosLiquidacion()
               oLiquidacion.ActualizarLugarDePagoyFecha(Me.txtLugarPago.Text,
                                                        Me.dtpSueldosFechaPago.Value,
                                                        Integer.Parse(Me.cmbTipoRecibo.SelectedValue.ToString()),
                                                        Integer.Parse(Me.cmbPeriodoLiquidacion.SelectedValue.ToString()),
                                                        Integer.Parse(cmbNroLiquidacion.SelectedValue.ToString()),
                                                        IdLegajo)

               MessageBox.Show("Los datos se modificaron correctamente! ", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Information)
