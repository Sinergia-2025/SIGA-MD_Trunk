...
#Region "Overrides"
   ...
   Protected Overrides Sub OnLoad(ByVal e As System.EventArgs)
      ...
      Try
         ...
         Me.cmbItems.SelectedIndex = 0
         Me.cmbCant.SelectedIndex = 0
         Me.cmbPrecios.SelectedIndex = 0
         Me.cmbFechaE.SelectedIndex = 0

         _tit = GetColumnasVisiblesGrilla(ugeResumen)
         Me.cmbActivadas.SelectedIndex = 0
...
#Region "Eventos"
   ...
   Private Sub btnConsultar_Click(sender As Object, e As EventArgs) Handles btnConsultar.Click
      Try
         ...
         If Me.chbObservacion.Checked AndAlso cmbObservaciones.SelectedIndex = -1 Then
            MessageBox.Show("ATENCION: NO seleccionó la observación aunque activó ese Filtro.", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Information)
            Me.cmbObservaciones.Focus()
            Exit Sub
         End If
   ...
   Private Sub chbObservacion_CheckedChanged(sender As Object, e As EventArgs) Handles chbObservacion.CheckedChanged
      ...
      Me.cmbObservaciones.Enabled = Me.chbObservacion.Checked
   ...
   Private Sub tsbRefrescar_Click(sender As Object, e As EventArgs) Handles tsbRefrescar.Click
      ...
      Me.chbObservacion.Checked = False
   ...
   Private Sub bscCodigoProducto2_BuscadorClick() Handles bscCodigoProducto2.BuscadorClick
      Try
         ...
         Me.bscCodigoProducto2.Datos = oProductos.GetPorCodigo(Me.bscCodigoProducto2.Text.Trim(), actual.Sucursal.Id,
                                                               Publicos.ListaPreciosPredeterminada, "TODOS",,,,,,,,,,,,, True)
      Catch ex As Exception
         ShowError(ex)
      End Try
   End Sub
   ...
   Private Sub bscProducto2_BuscadorClick() Handles bscProducto2.BuscadorClick
      Try
         ...
         Me.bscProducto2.Datos = oProductos.GetPorNombre(Me.bscProducto2.Text.Trim(), actual.Sucursal.Id,
                                                         Publicos.ListaPreciosPredeterminada, "TODOS",,,,,,,,,, True)
      Catch ex As Exception
         ShowError(ex)
      End Try
...
#Region "Métodos"
   ...
   Private Sub CargarGrillaDetalle()
      ...
      Dim FechaAutorizacionDesde As Date = Nothing
      Dim FechaAutorizacionHasta As Date = Nothing
      ...
      If Me.chbFechaAutorizacion.Checked Then
         FechaAutorizacionDesde = Me.dtpFechaAutorizacionDesde.Value
         FechaAutorizacionHasta = Me.dtpFechaAutorizacionHasta.Value
      End If
      ...
      If Me.chbObservacion.Checked Then
         idObservacion = Integer.Parse(Me.cmbObservaciones.SelectedValue.ToString())
      End If
      ...
      Dim oActivacionesOC As Reglas.ActivacionesOC = New Reglas.ActivacionesOC
      Dim dtDetalle As DataTable
      dtDetalle = oActivacionesOC.GetInforme(IdPedido, idProducto, Me.cmbActivadas.Text.ToString(),
                                             Me.cmbCriticidad.Text.ToString(), idObservacion, FechaDesde,
                                             FechaHasta, FechaDesdeEntrega, FechaHastaEntrega,
                                             FechaDesdeRepEntrega, FechaHastaRepEntrega,
                                             _tipoTipoComprobante, cmbTiposComprobantes.GetTiposComprobantes(),
                                             IdProveedor, IdUsuario, Me.cmbItems.Text.ToString(),
                                             Me.cmbCant.Text.ToString(), Me.cmbPrecios.Text.ToString(),
                                             Me.cmbFechaE.Text.ToString(), Me.cmbEstados.Text.ToString(),
                                             Me.chbVerUltimaActivacion.Checked, FechaAutorizacionDesde,
                                             FechaAutorizacionHasta)

      ugeResumen.DataSource = dtDetalle


      For Each dr As UltraGridRow In Me.ugeResumen.Rows
         If dr.Cells("Criticidad").Value.ToString() = "Normal" Then
            dr.Cells("Criticidad").Appearance.BackColor = Color.Green
         ElseIf dr.Cells("Criticidad").Value.ToString() = "Crítica" Then
            dr.Cells("Criticidad").Appearance.BackColor = Color.Red
         Else
            dr.Cells("Criticidad").Appearance.BackColor = Color.White
         End If
         ...
         If IsDate(dr.Cells("FechaReprogEntrega").Value.ToString()) Then
            ...
            ElseIf Date.Parse(dr.Cells("FechaReprogEntrega").Value.ToString()) < Date.Now Then
         ...
         dr.Cells("ImportePendiente").Value = Decimal.Round((Decimal.Parse(dr.Cells("ImporteTotal").Value.ToString()) * Decimal.Parse(dr.Cells("PorcPendiente").Value.ToString())) / 100, 2)
   ...
   Private Sub chbProducto_CheckedChanged(sender As Object, e As EventArgs) Handles chbProducto.CheckedChanged
      ...
      Me.bscCodigoProducto2.Tag = Nothing
      Me.bscProducto2.Text = String.Empty

      Me.bscCodigoProducto2.Focus()
   End Sub

   Private Sub cmbActivadas_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbActivadas.SelectedIndexChanged
      Try

         If Me.cmbActivadas.SelectedIndex = 1 Then
            Me.grpVerificaciones.Visible = True
         Else
            Me.grpVerificaciones.Visible = False
         End If

         Me.ugeResumen.DisplayLayout.Bands(0).Columns("Items").Hidden = Not Me.grpVerificaciones.Visible
         Me.ugeResumen.DisplayLayout.Bands(0).Columns("Cantidades").Hidden = Not Me.grpVerificaciones.Visible
         Me.ugeResumen.DisplayLayout.Bands(0).Columns("Precio").Hidden = Not Me.grpVerificaciones.Visible
         Me.ugeResumen.DisplayLayout.Bands(0).Columns("FechaE").Hidden = Not Me.grpVerificaciones.Visible
   ...
   Private Sub chbFechaAutorizacion_CheckedChanged(sender As Object, e As EventArgs) Handles chbFechaAutorizacion.CheckedChanged
      Me.chkMesCompletoFA.Enabled = Me.chbFechaAutorizacion.Checked
      ...
      If Not Me.chbFechaAutorizacion.Checked Then
         Me.chkMesCompletoFA.Checked = False
   ...
   Private Sub chkMesCompletoFA_CheckedChanged(sender As Object, e As EventArgs) Handles chkMesCompletoFA.CheckedChanged
      Dim FechaTemp As Date

      Try
         If chkMesCompletoFA.Checked Then

            FechaTemp = New Date(Me.dtpFechaAutorizacionDesde.Value.Year, Me.dtpFechaAutorizacionDesde.Value.Month, 1)
            Me.dtpFechaAutorizacionDesde.Value = FechaTemp

            'Voy 1 Mes para adelante a partir del dia 1, luego bajo 1 Segundo.
            FechaTemp = Me.dtpFechaAutorizacionDesde.Value.AddMonths(1).AddSeconds(-1)
            dtpFechaAutorizacionHasta.Value = FechaTemp

         End If

         Me.dtpFechaAutorizacionDesde.Enabled = Not Me.chkMesCompletoFA.Checked
         Me.dtpFechaAutorizacionHasta.Enabled = Not Me.chkMesCompletoFA.Checked

      Catch ex As Exception

         chkMesCompletoFA.Checked = False
         MessageBox.Show(ex.Message, Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Information)
