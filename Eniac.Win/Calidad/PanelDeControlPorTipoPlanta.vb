Public Class PanelDeControlPorTipoPlanta
...
#Region "Campos"
   ...
   Private _current As UltraGridRow = Nothing
...
#Region "Overrides"
   ...
   Protected Overrides Sub OnLoad(ByVal e As System.EventArgs)
      ...
      Try
         ...
         If Reglas.Publicos.CalidadUtilizaPagAutomaticoPanelControlPlanta Then

            '# Refresco automático
            Dim tiempoPaginado As Integer = 1
            Dim tiempoRefresco As Integer = Reglas.Publicos.CalidadPanelDeControlPlantaTiempoPaginado

            Me.txtMinutos.Value = tiempoRefresco
            Me.Automatico = Me.chbActualizacionAutomatica.Checked
            Me.Timer1.Interval = tiempoRefresco * 1000
            Me.Timer1.Enabled = Automatico
         End If
         'Me.ugDetalle.DisplayLayout.Override.ActiveAppearancesEnabled = DefaultableBoolean.False
         Me.ugDetalle.DisplayLayout.Override.ActiveRowAppearance.BackColor = Nothing
...
#Region "Metodos"
   ...
   Private Sub CargarGrillaDetalle()
      ...
      Dim pivotResult = regla.GetProductosPorListasControlTipos(If(chbProducto.Checked, bscCodigoProducto2.Text, String.Empty),
                                                                ...
                                                                Entidades.Publicos.SiNoTodos.NO, _ocultarFiltros, True)
      ...
      With ugDetalle.DisplayLayout.Bands(0)
         For Each drColumnas As DataRow In pivotResult.dtColumnas.Rows
            ...
            '.Columns(colName).CellAppearance.FontData.SizeInPoints = 14
            .Columns(colName).Header.Appearance.FontData.SizeInPoints = 11
            .Columns(colName).FormatearColumna(drColumnas("NombreListaControlTipo").ToString(), pos, 120, HAlign.Center, , "dd/MM/yyyy HH:mm")
...
#End Region
   ...
   Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
      Try
         If _current Is Nothing Then
            _current = ugDetalle.Rows(0)
            Me.CargarGrillaDetalle()
         Else
            '_current = ugDetalle.DisplayLayout.ActiveRow.GetSibling(SiblingRow.Next)
            ugDetalle.PerformAction(UltraGridAction.PageDownRow, False, False)
            _current = ugDetalle.DisplayLayout.ActiveRow
            ' If ugDetalle.DisplayLayout.ActiveRow.GetSibling(SiblingRow.Next) Is Nothing Then
            '_current = Nothing
            'Else
            If ugDetalle.DisplayLayout.ActiveRow.Index = ugDetalle.Rows.GetFilteredInNonGroupByRows.Last.Index Then
               _current = Nothing
            Else
               _current.Activate()
            End If
   ...
   Private Sub txtMinutos_ValueChanged(sender As Object, e As EventArgs) Handles txtMinutos.ValueChanged
      Try
         If Reglas.Publicos.CalidadUtilizaPagAutomaticoPanelControlPlanta Then
            Me.Intervalo = CInt(Me.txtMinutos.Value) * 1000
         End If
      Catch ex As Exception
         ShowError(ex)
      End Try
