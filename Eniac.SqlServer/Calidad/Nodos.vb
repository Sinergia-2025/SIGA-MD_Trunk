...
#Region "Insert/Update/Merge/Delete"
   ...
   Public Sub Nodos_M(idNodo As Integer,
                                        nombreNodo As String)
      Dim myQuery As StringBuilder = New StringBuilder()
      With myQuery
         .AppendFormatLine("MERGE INTO CalidadNodos AS D")
         .AppendFormatLine("        USING (SELECT  {1}  AS {0}", Entidades.Nodo.Columnas.IdNodo.ToString(), idNodo)
         .AppendFormatLine("                    , '{1}' AS {0}", Entidades.Nodo.Columnas.NombreNodo.ToString(), nombreNodo)
         .AppendFormatLine("              ) AS O ON O.{0} = D.{0}",
                           Entidades.Nodo.Columnas.IdNodo.ToString())
         .AppendFormatLine("    WHEN MATCHED THEN")
         .AppendFormatLine("        UPDATE SET D.{0} = O.{0}",
                           Entidades.Nodo.Columnas.NombreNodo.ToString())
...
#Region "GetAll"
   Public Function Nodos_GA() As DataTable
      Return Nodos_G(idNodo:=0)
   End Function
   ...
   'End Function
   Private Function Nodos_G(idNodo As Integer?) As DataTable
      Dim myQuery As StringBuilder = New StringBuilder()
      With myQuery
         Me.SelectTexto(myQuery)
         .AppendLine(" WHERE 1 = 1")
         If idNodo <> 0 Then
            .AppendFormatLine("   AND CA.{0} =  {1} ", Entidades.Nodo.Columnas.IdNodo.ToString(), idNodo)
         End If
      End With
   ...
   End Function
   Public Function Nodos_G1(idNodo As Integer) As DataTable
      Return Nodos_G(idNodo)
   End Function
