...
DECLARE @IdTipoComprobanteOrigen as varchar(10) = 'LIQUIDO';
DECLARE @CentroEmisorOrigen as int = 1;
DECLARE @LetraOrigen as varchar(1) = 'M';

DECLARE @IdTipoComprobanteDestino as varchar(10) = 'LIQUIDO';	--No cambia
DECLARE @CentroEmisorDestino as int = 1;						--No cambia
DECLARE @LetraDestino as varchar(1) = 'M';

DECLARE @NumeroOrigen as int = 501;
DECLARE @NumeroDiferencia as int = 108-501+1;
...
DELETE CuentasCorrientes
 WHERE IdTipoComprobante = @IdTipoComprobanteOrigen
   ...
   AND CentroEmisor = @CentroEmisorOrigen
   AND NumeroComprobante >= @NumeroOrigen
   AND NumeroComprobante < @NumeroDiferencia
--   AND IdTipoComprobante NOT IN ('RECIBO','ANTICIPO')
;
