 ...
 WHERE EXISTS 
     (SELECT * FROM MovimientosCompras MC
       WHERE MC.IdSucursal=MovimientosComprasProductos.IdSucursal
         ...
         AND CONVERT(varchar(11), MC.FechaMovimiento, 120) < '2016-01-01')
GO

DELETE MovimientosCompras
  WHERE CONVERT(varchar(11), FechaMovimiento, 120) < '2016-01-01'
GO
 ...
 WHERE EXISTS 
     (SELECT * FROM Compras C
       WHERE C.IdSucursal=ComprasProductos.IdSucursal
         ...
         AND CONVERT(varchar(11), C.Fecha, 120) < '2016-01-01')
GO

DELETE Compras
  WHERE CONVERT(varchar(11), Fecha, 120) < '2016-01-01'
GO
 ...
 WHERE EXISTS 
     (SELECT * FROM MovimientosVentas MV
       WHERE MV.IdSucursal=MovimientosVentasProductos.IdSucursal
         ...
         AND CONVERT(varchar(11), MV.FechaMovimiento, 120) < '2016-01-01')
GO
 ...
 WHERE EXISTS 
     ( SELECT * FROM Ventas V
       WHERE V.IdSucursal = VentasTarjetas.IdSucursal
         ...
         AND CONVERT(varchar(11), V.Fecha, 120) < '2016-01-01')
GO
...
DELETE CajasDetalle
 WHERE EXISTS 
     ( SELECT * FROM Cajas C
       WHERE C.IdSucursal = CajasDetalle.IdSucursal
         ...
         AND CONVERT(varchar(11), C.FechaPlanilla, 120) < '2016-01-01')
 AND NOT EXISTS 
 ...
 AND ImporteCheques=0
GO

DELETE Cajas
 WHERE CONVERT(varchar(11), FechaPlanilla, 120) < '2016-01-01'
   AND NOT EXISTS 
     ( SELECT * FROM CajasDetalle Cd
       WHERE Cd.IdSucursal = Cajas.IdSucursal
         AND CD.IdCaja = Cajas.IdCaja
         AND CD.NumeroPlanilla = Cajas.NumeroPlanilla)
GO

DELETE HistorialPrecios
 WHERE CONVERT(varchar(11), FechaHora, 120) < '2016-01-01'
GO
...
DELETE UsuariosAccesos
 WHERE CONVERT(varchar(11), FechaAcceso, 120) < '2016-01-01'
GO
...
UPDATE ProductosSucursales 
 SET ProductosSucursales.StockInicial = ProductosSucursales.Stock + 
