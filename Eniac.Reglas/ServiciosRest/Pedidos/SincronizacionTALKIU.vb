...
Namespace ServiciosRest.TiendasWeb
   ...
   Public Class SincronizacionTALKIU
      Inherits Base

      Public serializer As New JavaScriptSerializer()

#Region "Propiedades"
      Private Property TALKIUURLBase As Uri
      ...
      Public Property _OC As Entidades.OrdenesTalkiu
...
#Region "Constructors"
      Private Sub New(accesoDatos As Datos.DataAccess)
         da = accesoDatos
      End Sub
      Public Sub New(ByVal OC As Long)
         Me.New(New Datos.DataAccess())
         If Not String.IsNullOrEmpty(Reglas.Publicos.PedidosTalkiuToken) Then
            TALKIUURLBase = New Uri(String.Format(Reglas.Publicos.PedidosTalkiuGET, OC.ToString()))
         Else
...
#Region "Methods"
      ...
      Private Sub _BajarPedidos()
         ...
         Dim response = GETResponse(Of Entidades.OrdenesTalkiu)(Me.GetHeaders(), "")

         _OC = response

         '--------------------------
         '      If response.Count > 0 Then RaiseEvent Avance(Me, New SincronizacionTiendaWebEventArgs(Entidades.TiendasWeb.MERCADOLIBRE, "Bajando pedidos Mercado Libre..."))
         '--------------------------
         '      My.Application.Log.WriteEntry(String.Format("SubidaMercadoLibre.Ejecutar - {0} Pedidos (posibles) por bajar.", response.Count), TraceEventType.Verbose)
         ...
         '  For Each pedido As Entidades.JSonEntidades.Pedidos.TALKIU.Ordenes In response

         '   '-- Si el pedido ya esta grabado, no lo voy a buscar.- --
         '   If rPedidosTiendasWeb.ExistePedido(pedido.proposal_id.ToString(), sistemaDestino) Then Continue For

         '   count += 1
         '   ePTW = New Entidades.PedidoTiendaWeb

         '   My.Application.Log.WriteEntry(String.Format("SubidaMercadoLibre.Ejecutar - Obteniendo Pedido {0}.", pedido.proposal_id), TraceEventType.Verbose)
         '   Dim p = GETResponse(Of Entidades.JSonEntidades.TiendasWeb.Mercadolibre.PedidosML)(Me.GetHeaders(), GetEndpoint(Controls.orders.ToString(), pedido.proposal_id.ToString()))

         '   TriggerEvent(SincroTiendaWebEstados.Bajando, SincroTiendaWebMetodos.GET, response.Count, count, "pedidos", countExitoso, countError)

         '   '# vuelvo a serializar el json y me lo guardo como auditoría
         '   Dim jsonstring As String = serializer.Serialize(p)

         '    With ePTW
         '   .Id = response.proposal_id.ToString()
         ...
         '   My.Application.Log.WriteEntry(String.Format("SubidaMercadoLibre.Ejecutar - Grabando Pedido {0}.", pedido.proposal_id), TraceEventType.Verbose)

         '   rPedidosTiendasWeb._Inserta(ePTW, soloTransaccion:=True)

         '   My.Application.Log.WriteEntry(String.Format("SubidaMercadoLibre.Ejecutar - Pedido {0} grabado correctamente.", pedido.proposal_id), TraceEventType.Verbose)
         ...
         'If _OC Is Nothing Then
         '   RaiseEvent Avance(Me, New SincronizacionTiendaWebEventArgs(Entidades.OrdenesTalkiu, "No hay pedidos para bajar."))
         'End If
         '-------------------------------------------------------------------------
      End Sub

      'Public Sub SubirProductos()
      '   '-- Publica Productos en Mercado Libre.- --
      '   EjecutaConConexion(Sub() _SubirProductos())
      '   '--------------------------
      'End Sub
      '      Public Sub _SubirProductos()
      ...
      '         Dim dt As DataTable = rProductos.GetProductosTiendaWeb(_reenviarTodo, Entidades.TiendasWeb.MERCADOLIBRE)
      '         Dim count = 0I
      '         Dim countExitoso = 0I
      '         Dim countError = 0I
      ...
      'Public Sub SubirInformacion(reenviarTodo As Boolean, sincronizaProductos As Boolean)
      ...
      '      If sincronizaProductos Then SubirProductos()
      '      '--------------------------
      '   End If
      ...
      '   Dim uri = New Uri(Me.TALKIUURLBase, relativeUri)
      '   '--------------------------------------------------------------------------------------
...
#Region "Web Methods"
      Public Function GETResponse(Of T)(headers As Dictionary(Of String, String), relativeUri As String) As T
         Dim uri As Uri = New Uri(Me.TALKIUURLBase, relativeUri)
         ...
         Try
            If headers IsNot Nothing Then
               ...
               Next
            End If
            Dim rootObject As Entidades.OrdenesTalkiu
            Using orResult As System.IO.Stream = wc.OpenRead(uri)
               Using reader As StreamReader = New StreamReader(orResult, New System.Text.UTF8Encoding())
                  ...
                  Dim pepe As T = DirectCast(serializer.Deserialize(responseString, GetType(T)), T)
      ...
      Private Function GetHeaders(since_id As Long, onlyIds As Boolean) As Dictionary(Of String, String)
         ...
         headers.Add("Authorization", String.Format("Bearer {0}", Reglas.Publicos.PedidosTalkiuToken))
         Return headers
      End Function
